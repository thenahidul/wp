...
class WP_Bootstrap_Comments_Walker extends Walker_Comment {
     ...
     *
     * Bootstrap media element requires child comments to be nested within the parent comment-content clearfix.
    ...
    protected function html5_comment( $comment, $depth, $args, $is_parent = false )
    {
        ...
        $comment_classes[] = 'comment even thread-even depth-1';
        ...
        if ( $comment->comment_parent > 0 ) {
            ...
            $comment_classes[] = 'has-parent comment byuser comment-author-_smcl_admin odd alt depth-2';
...
?>
        <<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( $class_str, $comment ); ?>>
            ...
            <div id="div-comment-<?php comment_ID(); ?>" class="comment-wrap clearfix">
                ...
                <div class="comment-content clearfix">
                    ...
                    <div class="comment-author">
                        <?php printf( __( '%s <span class="says sr-only">says:</span>' ), sprintf( '%s', get_comment_author_link( $comment ) ) ); ?>
                        <span>
    ...
    protected function get_comment_author_avatar( $comment, $args )
    {
        ...
        if ( '' !== $comment_author_url ) {
            $avatar_string = sprintf(
                '<a href="%1$s" class="author-link url avatar avatar-60 photo avatar-default" rel="external nofollow">%2$s</a>',
